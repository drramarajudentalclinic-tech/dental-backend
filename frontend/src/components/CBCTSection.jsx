 ...
 * CBCTSection.jsx
 * 
 * Drop this into your existing ImageUpload component as the CBCT tab/section.
 * It handles:
 *   1. Listing existing CBCT volumes for the visit
 *   2. Uploading a new ZIP → shows progress → adds to list
 *   3. "Open CBCT" button → launches the full CBCTViewer overlay
 *
 * Props:
 *   visitId  {number}  Current visit ID
 *   apiBase  {string}  e.g. "/api"
 *
 * Usage (in your ImageUpload.jsx CBCT tab):
 *   import CBCTSection from "./CBCTSection";
 *   <CBCTSection visitId={visitId} apiBase="/api" />
 */

import React, { useState, useEffect, useRef, useCallback } from "react";
import CBCTViewer from "./CBCTViewer";

const API = "/api";   // change if needed, or pass as prop
...
export default function CBCTSection({ visitId, apiBase = API }) {
  ...
  const loadVolumes = useCallback(() => {
    setLoading(true);
    fetch(`${apiBase}/visits/${visitId}/cbct`)
      .then(r => r.json())
  ...
  const doUpload = useCallback(async (file) => {
    ...
    try {
      ...
      setUploadPct(100);
      setUploadMsg(`✅ Done! ${data.num_slices} axial slices processed.`);
      setTimeout(() => {
        setUploading(false);
        setUploadPct(0);
        setUploadMsg("");
  ...
  const deleteVolume = async (id) => {
    ...
    await fetch(`${apiBase}/cbct/${id}`, { method: "DELETE" });
    loadVolumes();
  };
  ...
  // ── CBCT viewer open ─────────────────────────────────────────────────────
  if (openVolumeId !== null) {
    return (
      <CBCTViewer
        volumeId={openVolumeId}
        apiBase={apiBase}
        onClose={() => setOpenVolumeId(null)}
      />
    );
  }
...
const s = {
  ...
  dropZoneActive: {
    borderColor: BLUE, background: "#1d4ed811",
  },
  ...
  progressBar: {
    height: 6, background: BORDER, borderRadius: 4, overflow: "hidden", marginBottom: 12,
  },
  progressFill: {
    height: "100%", background: BLUE, transition: "width 0.4s ease", borderRadius: 4,
  },
  ...
  legend: {
    ...
    background: "#f8fafc04", border: `1px solid ${BORDER}`,
